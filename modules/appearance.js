...
// Tracking variable for cleanup
let appearanceSyncInterval = null;

async function loadAppearanceData(el, index) {
    const content = el.querySelector('.content');
    const { sub, type } = el.dataset;

    // Safety: Clear any existing sync loops before starting a new one
    if (appearanceSyncInterval) clearInterval(appearanceSyncInterval);
