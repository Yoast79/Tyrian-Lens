...
async function initCore() {
    ...
    // Explicitly clear account memory before new sync to prevent account-bleed
    accountData = {};
...
function buildMenu() {
    ...
    for (const main of sortedKeys) {
        ...
        mainEl.querySelector('.header').onclick = () => {
            ...
            // GUARD: Prevent search clicks from saving to local storage
...
async function loadCategoryData(el) {
    ...
    // Prevent reloading if already populated (crucial for mass-search)
    if (content.children.length > 0 && !content.querySelector('.loading-text')) return;
...
function createSlot(item, count = null) {
    ...
    // Add data-name so wardrobe.html can search it
    if (name) slot.setAttribute('data-name', name.toLowerCase());
...
// Global functions for wardrobe.html search orchestration
window.expandAllForSearch = async function() {
    const allHeaders = document.querySelectorAll('.main-cat.collapsed > .header, .sub-cat.collapsed > .header, .weight-cat.collapsed > .header');
    allHeaders.forEach(h => {
        h.click(); 
    });
};

window.rebuildMenu = buildMenu;

// Updated Tab handlers to re-trigger search if active
UI.unlockedBtn.onclick = () => { 
    if (currentTab === 'unlocked') return; 
    currentTab = 'unlocked'; 
    UI.unlockedBtn.classList.add('active'); 
    UI.lockedBtn.classList.remove('active'); 
    buildMenu(); 
    if (window.triggerSearch) setTimeout(window.triggerSearch, 50);
};

UI.lockedBtn.onclick = () => { 
    if (currentTab === 'locked') return; 
    currentTab = 'locked'; 
    UI.lockedBtn.classList.add('active'); 
    UI.unlockedBtn.classList.remove('active'); 
    buildMenu(); 
    if (window.triggerSearch) setTimeout(window.triggerSearch, 50);
};
